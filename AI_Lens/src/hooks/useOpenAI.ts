...
subsequent explanations. Even if this makes the analysis of the page and its ideas quite extensive, give it the necessary attention.
  ...
  const preloadPageAI = async (pageNum: number, force: boolean = false) => {
    if (!pdfDoc) return;
    if (!force && requestedPagesRef.current.has(pageNum)) {
      return;
    }
    if (force && requestedPagesRef.current.has(pageNum)) {
  ...
  const sendCurrentPage = async (pageNum: number) => {
    await preloadPageAI(pageNum, true);
  };
